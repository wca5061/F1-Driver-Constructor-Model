...
library(patchwork)
library(glue)
library(xtable)       # for the table print
...
drivers_focus <- c(
  "hamilton","bottas","norris","sainz","leclerc",
  "max_verstappen","perez","alonso","raikkonen",
  "giovinazzi","vettel","gasly"
)
...
first_year_per_driver <-
  f1_dat %>%
  group_by(driver) %>%
  summarize(first_year = first(year), .groups = "drop")

driver_mean_long <-
  driver_mean %>%
  pivot_longer(
    - .draw, names_to = "driver_id", values_to = "Skill",
...
driver_form_long <-
  driver_form %>%
  pivot_longer(
    - .draw, names_to = c("driver_id", "season_num"), values_to = "Form",
    names_pattern = "\\[(\\d+),(\\d+)\\]", names_transform = as.integer
  ) %>%
  mutate(
    Driver = as_factor(levels(f1_dat$driver)[driver_id]),
...
plt_skill_trajectory <-
  ...
  scale_fill_brewer(palette = "Set2", guide = "none") +   # REPLACED scale_fill_fira
  scale_colour_brewer(palette = "Dark2", guide = "none") +# REPLACED scale_colour_fira
  theme_minimal(base_size = 12) +                         # REPLACED theme_fira
  facet_wrap(~ Driver) +
  labs(
    x = "Season", y = "Skill (log odds ratio)",
    title = "F1 driver skill trajectories",
    subtitle = "Hybrid-era (2014–2021) driver skill,\naccounting for yearly constructor advantage."
  )

ggsave("img/plt_skill_trajectories.png", plot = plt_skill_trajectory,
...
plt_driver_skill_2021 <-
  ...
  geom_pointrange(colour = "steelblue4") +                # REPLACED firaCols[3]
  theme_minimal(base_size = 12) +                         # REPLACED theme_fira
  labs(
    title = "2021 F1 driver skill",
    subtitle = "Accounting for yearly constructor advantage.",
    x = "Skill (log odds ratio)", y = "Driver"
  )

ggsave("img/plt_skill_2021.png", plot = plt_driver_skill_2021,
...
constructor_mean_long <-
  constructor_mean %>%
  pivot_longer(
    - .draw, names_to = "constructor_id", values_to = "Advantage",
    names_pattern = "\\[(\\d+)]", names_transform = as.integer
  ) %>%
...
constructor_form_long <-
  constructor_form %>%
  pivot_longer(
    - .draw, names_to = c("constructor_id", "season_num"), values_to = "Form",
    names_pattern = "\\[(\\d+),(\\d+)\\]", names_transform = as.integer
  ) %>%
  mutate(
    Constructor = as_factor(levels(f1_dat$constructor)[constructor_id]),
    Year        = season_num + 2013
  )
...
plt_advantage_trajectory <-
  ...
  mutate(
    Constructor = fct_relevel(Constructor, "ferrari", "mercedes", "red_bull", "mclaren", "haas", "williams"),
  ...
  labs(
    x = "Season", y = "Advantage (log odds ratio)",
    title = "F1 constructor advantage trajectories",
    subtitle = "Hybrid-era (2014–2021) constructor advantage,\naccounting for yearly driver skill."
  )

ggsave("img/plt_advantage_trajectory.png", plot = plt_advantage_trajectory,
       width = 12, height = 9, bg = "white")

constructors_2021 <- c(
  "alfa","alphatauri","alpine","ferrari","haas","mclaren",
...
plt_advantage_avg <-
  ...
  geom_pointrange(colour = "steelblue4") +               # REPLACED firaCols[1]
  theme_minimal(base_size = 12) +                        # REPLACED theme_fira
  labs(
    title = "Average hybrid-era F1 constructor advantage",
    subtitle = "Accounting for yearly driver skill and constructor form.",
    x = "Advantage (log odds ratio)", y = "Constructor"
  )

ggsave("img/plt_advantage_avg.png", plot = plt_advantage_avg,
       width = 9, height = 6, bg = "white")

# Driver versus constructor contributions ----
# Extract scalar SD draws as numeric vectors
get_sd_draws <- function(par) {
  posterior::as_draws_df(fit$draws(par)) %>%
    dplyr::select(starts_with(par)) %>%
    dplyr::pull()
}

sd_driver         <- get_sd_draws("tau_driver")
sd_driver_season  <- get_sd_draws("tau_driver_season")
sd_team           <- get_sd_draws("tau_team")
sd_team_season    <- get_sd_draws("tau_team_season")
...
print(xtable::xtable(ranef_summary), type = "latex")wa

# Variance decomposition
var_driver <- sd_driver^2 + sd_driver_season^2
var_team   <- sd_team^2   + sd_team_season^2

# proportion due to car vs. driver
